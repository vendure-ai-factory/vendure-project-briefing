...
async function run() {
    ...
    const loginRes = await fetch(API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'vendure-token': '8eav63ddas' },
        body: JSON.stringify({
            query: `
    ...
    const bodyLogin = await loginRes.json();
    console.log('Login Response:', JSON.stringify(bodyLogin));
    ...
    const fetchRes = await fetch(API_URL, {
        ...
        headers: {
            ...
            'vendure-token': 'm2a4v9k1x7', // AT channel
