...
export default function CheckoutFlow() {
  const { order, isGuest, countries, targetOrder } = useCheckout();
  ...
  const getInitialState = () => {
    ...
    // For guests, check if contact info is already partially filled (from session/order)
    if (isGuest) {
    ...
    const hasAddress = !!(order.shippingAddress?.streetLine1 && order.shippingAddress?.country);
    const isCountryUnsupported = hasAddress && !countries.find(c => c.name === order.shippingAddress?.country);

    // RESTORATION: DO NOT auto-complete steps. Let users see and confirm each step.
    // Exception: For guests, 'contact' can be auto-completed if email is present.
    ...
    // In Merge Mode, we also start at 'shipping' (Step 1 or Step 2 for guest).
    // The specific step components (ShippingAddressStep, DeliveryStep) will handle the locking UI.
