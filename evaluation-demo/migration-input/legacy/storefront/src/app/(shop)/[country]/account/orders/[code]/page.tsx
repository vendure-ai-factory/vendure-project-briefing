...
import { initiateOrderMerge } from "../actions";
...
export default async function OrderDetailPage(props: PageProps<'/account/orders/[code]'>) {
    ...
    // Determine if order can still be merged (added to)
    const canMerge = !order.customFields?.batchExportedAt &&
        (order.state === 'PaymentSettled' || order.state === 'PaymentAuthorized' || order.state === 'ArrangingPayment' || order.state === 'CollectingPayment');
                    ...
                    <div className="flex items-center gap-4">
                        {canMerge && (
                            <form action={async () => {
                                'use server';
                                if (order.code) {
                                    await initiateOrderMerge(order.code);
                                }
                            }}>
                            ...
                            <CardContent>
                                {order.shippingLines.map((line, idx) => (
                                    <div key={idx} className="space-y-1 text-sm">
                                        <p className="font-medium">
                                            {/* @ts-ignore - shippingMethod was removed from query to prevent crash */}
