...
export default function ShippingAddressStep({ onComplete }: ShippingAddressStepProps) {
  ...
  // Note: We use a state that updates when country changes
  const [isManualCity, setIsManualCity] = useState(false);

  useEffect(() => {
    // Check if current country has data
    const hasData = !!ADDRESS_DATA[watchedCountryCode];
    // If country changes, we generally reset to dropdown if data exists
    // But we shouldn't wipe the city if it's already there and valid?
    // For simplicity, if switching countries, we reset mode.
    if (hasData) {
      setIsManualCity(false);
    } else {
      setIsManualCity(true);
    }
    // Note: We are NOT clearing the city value here to avoid annoying user if they accidently switch country
    // But logically, a city from Germany is invalid in Austria.
    // Ideally we should clear it if it doesn't match, but let's leave it to user to change.
  }, [watchedCountryCode]);
