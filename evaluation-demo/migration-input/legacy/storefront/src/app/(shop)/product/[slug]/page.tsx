...
export async function generateMetadata({
    ...
    const result = await getProductData(decodedSlug);
    const product = result.data.product;

    // ... (rest of the metadata logic remains similar but simplified)
...
export default async function ProductDetailPage({ params, searchParams }: PageProps<'/product/[slug]'>) {
    ...
    // Fetch Nail Profiles Server-Side to Bypass Client Cookie Restrictions
    const { fetchNailProfiles } = await import('@/lib/vendure/nail-api');
    const initialProfiles = await fetchNailProfiles();
    ...
    return (
        <>
            <div className="container mx-auto px-4 py-8 mt-16">
                <ProductView product={product} searchParams={searchParamsResolved} initialProfiles={initialProfiles} />
            </div>
