...
async function RegisterContent({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
    ...
    const countryCode = cookieStore.get('country_code')?.value || 'DE'; // Default to DE if missing (fallback)

    return <RegistrationForm redirectTo={redirectTo} countryCode={countryCode} />;
}
