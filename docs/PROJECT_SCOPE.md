...
See [Pipeline Implementation Reference](PIPELINE_IMPLEMENTATION_REFERENCE.md) for the shared proposal language: natural-language goal compilation, isolated Git-baselined execution, bounded automatic debugging, evidence-linked browser/API/database validation, independent PASS/BLOCK derivation, and separate code/data rollback. The reference does not mandate OpenHands, ClawAI, Docker, GitHub Actions, or any other named tool.
